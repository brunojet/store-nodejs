...
router.get('/health', async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    const status: 'healthy' | 'unhealthy' = result.status === 'healthy' || result.status === 'degraded' ? 'healthy' : 'unhealthy';
    ...
    logWithContext(
      ...
      result.status === 'healthy' ? 'info' : result.status === 'degraded' ? 'warn' : 'error',
